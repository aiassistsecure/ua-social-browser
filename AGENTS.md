...
| Instagram | Driven, upload-first, and **route-driven**: `composeUrl` is `/create/select/`, so nothing depends on finding a navigation control. **Verified 2026-09-03:** the owner watched one real post land on a real account (`paper_bagboys`) from the macOS shell — image and caption both present in the feed. The UA profile selects the graph (`publisher/device.ts`), because Instagram serves a phone a different flow with different elements and different URLs. That post was made under the **Chrome · macOS** profile; **the mobile graph has never been watched working.** Three of the original selectors were wrong in ways reading could not catch — see `.agents/memory/attachments.md`. |
| Pinterest | Driven, upload-first, single-screen: the flow waits for the upload control, attaches, and only then types the description. Refuses a post with no attachment. **Selectors unverified, same as the six above.** Does not model board selection — see `adapters.ts`. |
