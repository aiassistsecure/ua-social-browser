...
- **Driven end to end** — X and Instagram are the two verified with real posts on real accounts (X on 2026-09-02, Instagram on 2026-09-03, both from the macOS shell). LinkedIn, Facebook, Threads, Bluesky, Mastodon, Tumblr and Pinterest are driven by the same flow but have never been run against a signed-in account. Instagram and Pinterest are upload-first: the picture goes in before the caption, because there is no caption field until an image is in. The shell opens the composer inside the workspace's own session, types the approved text, submits, and waits for the network's own confirmation.
- **The UA profile decides which composer to drive.** Instagram serves a phone a different flow — different elements, different controls, different URLs — so the workspace's declared device picks the graph rather than one being assumed. Instagram's is reached by URL rather than by clicking navigation, which is the part that differs between layouts.
- **Refused, with the actual reason** — TikTok and YouTube need a video and an upload wizard this build does not drive; Reddit needs a community and a title the draft model does not have. Those answer with that specific reason and a link to post in the tab. The draft stays approved either way.
